...
vec4 CalcLightInternal(BaseLight Light, vec3 LightDirection, vec3 Normal)
{
	...
	// diffuse color
	float diffuseFactor = max(0.0, dot(Normal, -LightDirection));		//light direction wheather negative?
	vec4 diffuseColor = vec4(Light.Color, 1.0f) * Light.DiffuseIntensity * diffuseFactor;
	// specualr color
	vec3 vertexToEye = normalize(eyeWorldPos - vPosInViewSpace);
	vec3 reflectLight = normalize(reflect(LightDirection, Normal));		//light direction wheather negative?
	float specFactor = max(0.0, dot(vertexToEye, reflectLight));
	if (diffuseFactor > 0)
	{
		specFactor = pow(specFactor, specularPower);
	}
	vec4 specColor = vec4(Light.Color * specularIntensity * specFactor, 1.0f);
